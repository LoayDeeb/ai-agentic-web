...
const breadcrumbs = [
	...
	{ label: 'IMTIAZ Platinum', href: '/bahraincredit/cards/platinum' },
	{ label: 'IMTIAZ Platinum Credit Card', href: null },
]

export default function TamkeenBahrainPlatinumCard() {
	const navigate = useNavigate()
	const [openSections, setOpenSections] = React.useState<Record<string, boolean>>({ benefits: true })
	...
	React.useEffect(() => {
		const unsubscribe = onToolEvent((tool, args) => {
			if (tool === 'scrollToTamkeenSection' && args.sectionId) {
				...
				window.setTimeout(() => {
					document.getElementById(`tamkeen-platinum-${sectionId}`)?.scrollIntoView({
						behavior: 'smooth',
						block: 'start',
					})
				}, 80)
				...
				<div className="mx-auto max-w-[1170px]">
					<div className="flex flex-col items-start gap-0 md:flex-row">
						<aside className="order-2 mt-6 w-full flex-shrink-0 pr-0 md:order-1 md:mt-0 md:w-[25%] md:pr-4">
							<div className="mb-8">
								<ul className="m-0 list-none p-0">
									{platinumSidebarLinks.map((link, index) => (
										<li
											key={link.label}
											...
											style={{
												...
												marginBottom: index === platinumSidebarLinks.length - 1 ? 0 : 3,
											}}
										>
											<a
												href={link.path || '#'}
						...
						<main className="order-1 w-full flex-1 pl-0 md:order-2 md:w-[75%] md:pl-4">
							...
							>
								<img
									src="https://www.bahraincredit.com.bh/Administrator/MediaHandler/ImageHandler/images/PhotoGallery/Thumbnails/platinum.png"
									alt="IMTIAZ Platinum Mastercard"
							...
							>
								...
								>
									Welcome to IMTIAZ Platinum Mastercard®
								</h2>
								<p className="m-0 text-sm leading-relaxed opacity-90" style={{ color: TAMKEEN_PRIMARY }}>
									A refined premium card for customers who want stronger travel and lifestyle value
									with a more balanced profile than the World tier.
								</p>
							</div>

							{platinumSections.map((section) => {
								const Icon = section.icon
								const isOpen = Boolean(openSections[section.id])
								return (
									<div
										key={section.id}
										id={`tamkeen-platinum-${section.id}`}
							...
							<div className="mb-8 mt-2 flex flex-wrap justify-center gap-3">
								{cardFooterButtons.map((button) => (
									<a
										...
										onClick={(e) => {
											...
											if (button.label === 'Apply for IMTIAZ') {
												navigate('/bahraincredit/cards/apply?card=platinum')
											}
										}}
									...
									>
										{button.icon}
										{button.label === 'Apply for IMTIAZ' ? 'Apply for IMTIAZ Platinum' : button.label}
									</a>
								))}
							</div>
