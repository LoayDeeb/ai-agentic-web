...
const breadcrumbs = [
	...
	{ label: 'IMTIAZ for Her World', href: '/bahraincredit/cards/for-her-world' },
	{ label: 'IMTIAZ for Her World Credit Card', href: null },
]

export default function TamkeenBahrainForHerWorldCard() {
	const navigate = useNavigate()
	const [openSections, setOpenSections] = React.useState<Record<string, boolean>>({ travel: true })
	...
	React.useEffect(() => {
		const unsubscribe = onToolEvent((tool, args) => {
			if (tool === 'scrollToTamkeenSection' && args.sectionId) {
				...
				window.setTimeout(() => {
					document.getElementById(`tamkeen-for-her-${sectionId}`)?.scrollIntoView({
						behavior: 'smooth',
						block: 'start',
					})
				}, 80)
					...
					<div className="flex flex-col items-start gap-0 md:flex-row">
						<aside className="order-2 mt-6 w-full flex-shrink-0 pr-0 md:order-1 md:mt-0 md:w-[25%] md:pr-4">
							<div className="mb-8">
								<ul className="m-0 list-none p-0">
									{forHerWorldSidebarLinks.map((link, index) => (
										<li
											key={link.label}
											...
											style={{
												...
												marginBottom: index === forHerWorldSidebarLinks.length - 1 ? 0 : 3,
											}}
										>
											<a
												href={link.path || '#'}
						...
						<main className="order-1 w-full flex-1 pl-0 md:order-2 md:w-[75%] md:pl-4">
							...
							<div className="mb-5">
								...
								>
									IMTIAZ for Her World Mastercard®
								</h1>
							</div>
							...
							>
								<img
									src="https://www.bahraincredit.com.bh/Administrator/MediaHandler/ImageHandler/images/PhotoGallery/Thumbnails/IFHWorld.png"
									alt="IMTIAZ for Her World Mastercard"
									className="block w-full bg-white object-contain p-8"
									style={{ maxHeight: 240 }}
								/>
							</motion.div>
							...
							>
								...
								>
									Welcome to IMTIAZ for Her World Mastercard®
								</h2>
								<p className="m-0 text-sm leading-relaxed opacity-90" style={{ color: TAMKEEN_PRIMARY }}>
									A premium women-focused World card designed for customers who want stronger travel
									privileges, premium rewards, and a more polished everyday experience.
								</p>
							</div>

							{forHerWorldSections.map((section) => {
								const Icon = section.icon
								const isOpen = Boolean(openSections[section.id])
								return (
									<div
										key={section.id}
										id={`tamkeen-for-her-${section.id}`}
							...
							<div className="mb-8 mt-2 flex flex-wrap justify-center gap-3">
								{cardFooterButtons.map((button) => (
									<a
										...
										onClick={(e) => {
											...
											if (button.label === 'Apply for IMTIAZ') {
												navigate('/bahraincredit/cards/apply?card=for-her-world')
											}
										}}
									...
									>
										{button.icon}
										{button.label === 'Apply for IMTIAZ' ? 'Apply for IMTIAZ for Her World' : button.label}
									</a>
								))}
							</div>
