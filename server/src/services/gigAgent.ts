...
const tools: OpenAI.Chat.Completions.ChatCompletionTool[] = [
	...
	},
	{
		...
		function: {
			name: 'openGigHome',
			description: 'Open the GIG Jordan demo landing page for the Crown Family insurance experience',
			parameters: {
				type: 'object',
				properties: {},
				required: []
	...
	},
	{
		...
		function: {
			name: 'openGigSubmit',
			description: 'Open the local GIG Crown Family request form in the demo',
			parameters: {
				type: 'object',
				properties: {},
				required: []
			...
			parameters: {
				...
				properties: {
					step: {
						...
						minimum: 1,
						maximum: 3,
						description: 'Step number in the local GIG form'
					}
				},
				required: ['step']
			}
	...
	},
	{
		...
		function: {
			name: 'getFormData',
			description: 'Get the current local GIG form data and missing fields',
			parameters: {
				type: 'object',
				properties: {},
				required: []
	...
	},
	{
		...
		function: {
			name: 'submitForm',
			description: 'Submit the current local GIG request form after confirmation',
			parameters: {
				type: 'object',
				properties: {},
				required: []
...
const systemPrompt = `أنت المساعد الصوتي الخاص بعرض جي آي جي الأردن ضمن هذا الديمو.

الدور:
- ساعد المستخدم في استكشاف نسخة الديمو الخاصة ببرنامج كراؤن عائلتي.
- ركز فقط على صفحات جي آي جي في هذا المشروع: /gig و /gig/crown-family.
- إذا سأل المستخدم عن المصدر الرسمي أو أراد الانتقال للموقع الحقيقي فاستخدم أداة openGigOfficial.
...
- في هذا الديمو يوجد أيضاً نموذج محلي لطلب الاهتمام ببرنامج كراؤن عائلتي على المسار /gig/submit.

معلومات المنتج الأساسية داخل هذا الديمو:
- البرنامج هو كراؤن عائلتي مع مفهوم Unlimited coverage.
...
- إذا طلب المستخدم التعرف على ديمو جي آي جي أو بدء الاستكشاف فاستخدم openGigHome.
- إذا طلب تفاصيل التغطية أو الأسعار أو المنافع فاستخدم openGigCrownFamily.
- إذا طلب التقديم أو بدء الطلب أو تعبئة نموذج فاستخدم openGigSubmit.
- إذا طلب قسماً محدداً من صفحة التفاصيل فاستخدم scrollToGigSection.
- إذا طلب الصفحة الرسمية أو رابط جي آي جي الحقيقي فاستخدم openGigOfficial.
...
- إذا قال "تفاصيل كراؤن عائلتي" أو "التغطيات" أو "الأسعار" فاستخدم openGigCrownFamily.
...
قيود مهمة:
- لا تخترع تغطيات أو شروطاً غير الموجودة في الديمو.
- لا تعد المستخدم بإصدار وثيقة نهائية أو قبول تأميني فوري؛ هذا الديمو يجمع بيانات الاهتمام فقط.
- إذا طلب المستخدم التقديم فابدأ بالنموذج المحلي أولاً، ولا تحوله للموقع الرسمي إلا إذا طلب ذلك صراحة.
