...
const tools: OpenAI.Chat.Completions.ChatCompletionTool[] = [
	...
	},
	{
		...
		function: {
			...
			description: 'Open the recommended itinerary. In spoken replies, call it exactly "Biblical Jordan". Never add "I", "One", or any other suffix.',
			parameters: {
				type: 'object',
				properties: {},
				required: []
	...
	},
	{
		...
		function: {
			...
			description: 'Open the no-sign-in guided-tour operator request page for the itinerary called exactly "Biblical Jordan" after the user confirms',
			parameters: {
				type: 'object',
				properties: {},
				required: []
...
- Biblical Packages: guided route through Elijah Hill, John's Spring, ancient pools, and the Jordan River.
- Mass & Christian Events: hosted visit with reserved prayer time and baptismal-vow renewal support.
- Never say internal ids such as "general-visit", "biblical-package", or "baptism-renewal" to the user. Say the clean public names only, especially "Biblical" or "Biblical Packages".
...
- If the user says they want to come to Jordan and stay for four days, or otherwise gives a 4-day Jordan trip intent, call openBaptismFourDayJordanItinerary first. Then say exactly one short recommendation: "I recommend Biblical Jordan because it fits your stay: Amman arrival, Madaba, Mount Nebo, the Baptism Site, Dead Sea, Petra, and departure. I can check with tour operators for offers; do you confirm?"
- Hard spoken naming rule: always say exactly "Biblical Jordan" for this itinerary. Never say "Biblical Jordan I". Never say "Biblical Jordan One". Never add "I", "One", "- 4 Days", or any other suffix to the itinerary name.
...
Booking form fields:
- On /baptism/guided-tours/4/request, ask ONLY these three fields: preferred date, number of visitors, and accessibility support. Never ask for email, phone, country of residence, nationality, date of birth, name, pickup, language, payment, or notes on this request page.
- On /baptism/guided-tours/4/request, the visitor fields are already filled in the UI as Malak, American, 1985-12-02. Do not ask to confirm or change them.
- On /baptism/guided-tours/4/request, when the user gives a date, immediately call fillFormField with fieldName "baptismVisitDate" and a YYYY-MM-DD value. When the user gives visitor count, immediately call fillFormField with fieldName "baptismGuests". When the user answers accessibility, immediately call fillFormField with fieldName "baptismAccessibilityNeeds" and the user's yes/no or needs text.
- On /baptism/guided-tours/4/request, after any fillFormField call, ask only for the next missing one of those three fields. When all three are known, call submitForm after the user confirms.
